...
// How many fully-received packets we can hold before comms_read() must
// be called to make room. Separate from UART's own byte-level ring buffer
#define PACKET_BUFFER_LENGTH  (8)

// The three-state packet receiver: every incoming byte moves this
// state machine forward by exactly one step.
...
static uint32_t packet_buffer_mask = PACKET_BUFFER_LENGTH - 1;    //ring buffer
...
//memcpy
static void comms_packet_copy(comms_packet_t* source, comms_packet_t* dest){
...
uint8_t comms_compute_crc(comms_packet_t* packet) {
    return crc8((uint8_t*)packet, PACKET_LENGTH - PACKET_CRC_BYTES); //one must imagine sysyphus happy
}