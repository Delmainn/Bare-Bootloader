...
uart.on('data', data => {
  ...
  while (rxBuffer.length >= PACKET_LENGTH) {
    const raw = rxBuffer.slice(0, PACKET_LENGTH);
    ...
    if (packet.crc !== computedCrc) {
      // Slide one byte and try again — don't bail.
      consumeFromBuffer(1);
      continue;
    }

    consumeFromBuffer(PACKET_LENGTH);
...
Logger.info('Firmware update request accepted');
